---
layout: single
author_profile: true
---

## About Me

I am a Mechanical Engineering student passionate about bridging the gap between hardware and software. I love building physical systems and optimizing them using modern programming languages and automation tools.

## Technical Skills

### 🛠️ Engineering & Hardware

* **CAD/Modeling:** SolidWorks, Autodesk Fusion 360
* **Analysis & Simulation:** ANSYS, FEA (Finite Element Analysis)
* **Manufacturing:** 3D Printing (FDM/SLA), CNC Milling

### 💻 Software & Automation

* **Languages:** Python (Data analysis, automation), MATLAB, C++ (Arduino)
* **Developer Tools:** Git, GitHub, VS Code
* **Hardware Integration:** Raspberry Pi, Arduino microcontrollers

## Featured Projects

* **Project 1:** Automated Mechanical System (Python + Arduino)
* **Project 2:** Mechanical Design & Finite Element Analysis (SolidWorks + ANSYS)
